...
- [x] Table 1: Dataset summary (bars, events, horizon, embargo, period) — generated by generate_remaining.py (Phase 10)
- [x] Table 2: Class distribution (crash/no-crash per split) — 5-fold purged CV on adaptive labels; crash rate 55–58% per fold (subset artefact)
- [x] Table 3: Main performance (6 models × 7 metrics + runtime) — RF best: PR-AUC=0.634±0.024, MCC=0.093±0.028
- [x] Table 4: Standard vs adaptive labeling comparison — label agreement 93.9%; adaptive slightly reduces crash count
- [x] Table 5: Purged-Embargo vs standard CV comparison — leakage inflation near-zero on 200K subset (expected; full-dataset run on HPC)
- [x] Table 6: Top features per model family — ML: bb_width/roll_skew top features; DL: gradient proxy (run Phase 8 on HPC for exact IG values)
- [x] Table 7: Computational feasibility (training/inference time) — ML timing from Phase 5; DL timing from full 5-fold run (RNN 289s/fold, LSTM 230s/fold, GRU 455s/fold on MPS); param counts: RNN 13,761 / LSTM 54,849 / GRU 41,153
- [x] Table 8: Robustness checks (alt horizon, barrier, features, embargo) — 8 settings, RF best model, 3-fold purged CV
- [x] Figure 2: PR curves for all 6 models — generated from last 30% holdout; ML AP: XGB=0.827, LGBM=0.784, RF=0.745; DL AP ~0.54
- [x] Figure 3: Model ranking comparison — grouped bar chart (6 metrics × 6 models, mean ± std)
- [x] Figure 4: Class distribution under standard vs adaptive
- [x] Figure 5: Performance difference (adaptive - standard)
- [x] Figure 6: Leakage inflation visualization
- [x] Figure 7: SHAP summary (best tree-based model)
- [x] Figure 8: Integrated Gradients summary — LSTM gradient×input attribution on 500-event sample; top features: bb_width, ema_deviation, volatility (run Phase 8 on HPC for full captum IG)
- [x] Figure 9: Local explanation for representative crash event
- [x] Figure 10: Sensitivity analysis across settings
...
- [x] Phase 7: evaluation.py — CODE COMPLETE + both experiments run (Table 4, Table 5, Figures 4-6 saved; leakage sanity check passed for LightGBM and RF; XGBoost inflation near-zero as expected on 200K subset)
- [ ] Phase 8: interpretability.py (SHAP + IG) — code exists; full captum IG run requires HPC; lightweight proxy IG via gradient×input saved as Figure 8
- [x] Phase 9: robustness.py — CODE COMPLETE + run in lightweight mode (8 settings × 3-fold purged CV on RF; Table 7 ML timing compiled + DL param counts; Table 8 robustness CSV saved; Figure 10 sensitivity PDF/PNG saved)
- [x] Phase 10: Final figures + tables — ALL 8 tables + ALL 9 figures generated (generate_remaining.py); notebooks/10_final.ipynb created; completeness check passed
...
| 2026-04-11 | Phase 9 robustness uses 3-fold CV (vs 5-fold in main experiments) for speed; RF is the best model from Phase 5 (PR-AUC=0.634). This is noted in paper as "robustness CV". | 8 settings × 5-fold = 40 RF fits would take ~30 min on M1 Air; 3-fold = 24 fits, ~15 min, acceptable. |
| 2026-04-11 | Robustness top-5 features by RF Gini importance (SHAP proxy): bb_width, log_return, volatility, ema_deviation, vwap_deviation. MCC stable at 0.087 vs 0.093 baseline (6% drop with 73% fewer features). | RF feature_importances_ is a reasonable proxy for SHAP when Phase 8 (SHAP) not yet run |
| 2026-04-11 | Narrow barriers (pt=1.0, sl=0.5) give best PR-AUC=0.666 and MCC=0.126; Wide barriers (pt=2.0, sl=1.5) give worst PR-AUC=0.608. More sensitive barriers → more labeled crashes → better signal for RF in lightweight mode (56% crash rate). Results will differ on full dataset with realistic crash rate. | In lightweight mode, high crash rate means narrow barriers increase precision by labeling more frequent low-barrier hits |
| 2026-04-11 | Embargo sensitivity: changing embargo_pct from 0.5% to 2.0% changes PR-AUC by only ±0.001 (0.634 baseline → 0.634 at both). Validates robustness of purged CV approach to embargo choice. | Confirms that the 1% embargo in CLAUDE.md is a safe default; results don't change materially with ±2× variation |